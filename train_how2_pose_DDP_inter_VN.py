...
class How2SignPoseDataset(Dataset):
    def __init__(self, arg_dict, tokenizer, phase='train', split='train'):
        ...
        self.label_path = "how2sign\dataset\how2sign_realigned_val.csv"

        print(f'Loading dataset from {self.label_path}...')
        ...
        with open('tools/how2sign_missing.txt', 'r') as f:
            names = f.readlines()
    ...
    def read_pose_files(self, index: int):
        ...
        file_path = os.path.join("how2sign\dataset\openpose_output\pkl", f'{vid_name}.pkl')
        ...
        #     return np.zeros((self.visual_token_num, 78, 3)), 0
...
def eval(dataset, model, args, device, split, save_results: bool = False,
    ...
    if args.local_rank == 0 and save_results:
        ...
        with open(os.path.join(output_dir, f"{output_prefix}_{split}_eval.csv"), 'w', encoding='utf-8') as f:
            for gen_text, label_text in zip(gen_text_list, ref_text_list):
                f.write(f"{gen_text}|{label_text}\n")
